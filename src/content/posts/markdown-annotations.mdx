...
连续写下两条或更多附记，并让它们共同紧贴一个正文段落时，这些附记会自动组成一叠卡片。正文中的 `:sidenote[]` 行内标记按出现顺序与卡片配对；标记可以嵌套，因此一张卡片可以对应整句，另一张只对应句中的一部分：
...
:sidenote[这是一整句话，其中 :sidenote[这一部分] 另有一条附记。]
```

:::sidenote[背景]
这是第一张卡片，用来补充这句话的背景信息。
:::

:::sidenote[出处]
这是第二张卡片，用来记录这句话的资料出处。
:::

:::sidenote[延伸]
这是第三张卡片，提供一个可以继续阅读的方向。
:::

:sidenote[这句话同时关联了 :sidenote[背景和出处]，也包含 :sidenote[延伸阅读]。] 每张卡片保留自己的颜色和文字范围；点击当前卡片可以切换到下一张，把鼠标移到对应文字上也会切换到那张卡片。
...
插件生成 `<aside role="note">`，并把它后面紧跟的段落标记为对应正文；连续附记会共享同一个正文段落并组成卡片叠。行内标记数量必须与卡片数量一致；如果省略全部行内标记，则兼容旧语法，让所有卡片共享整段正文。如果最后一条附记后没有正文段落，构建会直接报错。在手机或较窄的窗口中，单条附记会成为带有标题的正文块，多条附记仍可逐张切换；术语原文则继续显示在术语上方。

钢琴窗中的每个琴键都是带有按下状态的按钮，可以通过键盘聚焦并操作。琴键较多或屏幕较窄时，键盘区域可以横向滚动；RSS 中则保留标题与初始音符文字，不依赖交互脚本。

RSS 使用同一套 Markdown renderer，并保留每一张卡片对应的 `<aside>`、`<ruby>` 与 `<rt>` 语义；所有附记内容都会依次出现，因此不会依赖卡片切换脚本或泄漏未解析的 directive 源码。
