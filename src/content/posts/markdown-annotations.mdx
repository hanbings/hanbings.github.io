...
本站将复杂的富文本能力注册为全局 MDX 组件。文章可以直接使用 `<Underline>`、`<Term>`、`<Sidenote>`、`<SidenoteText>` 和 `<Piano>`，无需在每篇文章中重复导入。普通 Markdown 继续使用 Astro 默认渲染器，适合不需要组件的内容。

## MDX 手绘下划线

MDX 页面可以直接使用全局注册的 `<Underline>` 组件。默认使用站点强调色，也可以传入任意 CSS 颜色：

```mdx
<Underline>默认黄色手绘下划线</Underline>
<Underline color="#e88c9a" strokeWidth={2.5}>
  自定义颜色
</Underline>
```

例如，<Underline>这段文字使用默认的黄色手绘下划线</Underline>；而 <Underline color="#e88c9a" strokeWidth={2.5}>这段文字使用柔和的红色</Underline>。组件支持多行文字，并在减少动态效果模式下自动关闭绘制动画。
